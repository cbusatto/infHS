...
struct ihs_parallel_step0 : public RcppParallel::Worker {
  ...
  void operator()(std::size_t k_0, std::size_t k_1) 
  {
    ...
    for (j = k_0; j < k_1; j++) 
    {
      ...
      a_lambda[j] = std::min(0.5 * mu_beta2, 20.0);
      b_lambda[j] = s02inv;
      c_lambda[j] = 0.0;
...
struct ihs_pr_parallel_step : public RcppParallel::Worker {
  ...
  void operator()(std::size_t k_0, std::size_t k_1) 
  {
    ...
    for (j = k_0; j < k_1; j++) 
    {
      ...
      // tmp_j[j] = -lkf_p + lkf + a_lambda[j] * e3 + b_lambda[j] * e2 - c_lambda[j] * e1 - std::log(d_phi[j]);
      tmp_j[j] = lkf + a_lambda[j] * e3 + b_lambda[j] * e2 - c_lambda[j] * e1 - std::log(d_phi[j]);
    }
  }
};
...
struct ihs_parallel_step02 : public RcppParallel::Worker {
  // function that return the updated inverse matrix after updating m columns
  // Q, R starting QR decomposition
  // k number of trials
  // X data matrix
  ...
  ihs_parallel_step02 (const Eigen::VectorXd& mu_beta, const Eigen::VectorXd& dSigma, Rcpp::NumericVector& a_lambda, Rcpp::NumericVector& b_lambda, Rcpp::NumericVector& c_lambda, Rcpp::NumericVector& d_phi, Rcpp::NumericVector& mu_lambda, Rcpp::NumericVector& mu_phi, Rcpp::NumericVector& lambda_phi, Rcpp::NumericVector& bl_sum, Rcpp::NumericVector& tmp_j) 
    : mu_beta(mu_beta), dSigma(dSigma), a_lambda(a_lambda), b_lambda(b_lambda), c_lambda(c_lambda), d_phi(d_phi), mu_lambda(mu_lambda), mu_phi(mu_phi), lambda_phi(lambda_phi), bl_sum(bl_sum), tmp_j(tmp_j) {}
  ...
  void operator()(std::size_t k_0, std::size_t k_1) 
  {
    ...
    for (j = k_0; j < k_1; j++) 
    {
      ...
      a_lambda[j] = std::min(0.5 * mu_beta2, 18.0);
      b_lambda[j] = 0.5;
      c_lambda[j] = 0.0;
      
      double b_lambda_sq = std::sqrt(0.5);
      // double lkf = 2.0 * log_kf(a_lambda[j], b_lambda_sq, c_lambda[j]);
      // double e1 = 2.0 * m_e1(a_lambda[j], b_lambda_sq, c_lambda[j], lkf);
      // double e2 = 2.0 * m_e2(a_lambda[j], b_lambda_sq, c_lambda[j], lkf);
      // double e3 = 2.0 * m_e3(a_lambda[j], b_lambda_sq, c_lambda[j], lkf);
      double lkf = log_kf(a_lambda[j], b_lambda_sq, c_lambda[j]);
      double e1 = m_e1(a_lambda[j], b_lambda_sq, c_lambda[j], lkf);
      double e2 = m_e2(a_lambda[j], b_lambda_sq, c_lambda[j], lkf);
      double e3 = m_e3(a_lambda[j], b_lambda_sq, c_lambda[j], lkf);
      
      // Rcpp::Rcout << "ii: " << lkf << ", " << e1 << ", " << e2 << ", " << e3;
      bl_sum[j] = mu_beta2 * e3;
      ...
      // tmp_j[j] = -std::log(0.5) + lkf + 2.0 * a_lambda[j] * e3 + b_lambda[j] * e2 - std::log(d_phi[j]);
      tmp_j[j] = lkf + 2.0 * a_lambda[j] * e3 + 0.5 * e2 - std::log(d_phi[j]);
    }
  }
};
...
std::tuple<Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd> ihs_pstep02 (const Eigen::VectorXd& mu_beta, const Eigen::VectorXd& dSigma) 
{
  ...
  ihs_parallel_step02 Ihs_parallel_step0(mu_beta, dSigma, a_lambda, b_lambda, c_lambda, d_phi, mu_lambda, mu_phi, lambda_phi, bl_sum, tmp_j);
  RcppParallel::parallelFor (0, p-1, Ihs_parallel_step0);
...
struct ihs_parallel_step2 : public RcppParallel::Worker {
  // function that return the updated inverse matrix after updating m columns
  // Q, R starting QR decomposition
  // k number of trials
  // X data matrix
  ...
  const double mu_tau2, mu_sigma, mu_sigma2;
  ...
  ihs_parallel_step2 (const Eigen::MatrixXd& Z, const Eigen::VectorXd& mu_beta, const Eigen::VectorXd& dSigma, const Eigen::VectorXd& mu_gamma, const Eigen::MatrixXd& Sigma_gamma, const Eigen::VectorXd& dphi, const Eigen::VectorXd& muphi, const double& mu_tau2, const double& mu_sigma, const double& mu_sigma2, Rcpp::NumericVector& a_lambda, Rcpp::NumericVector& b_lambda, Rcpp::NumericVector& c_lambda, Rcpp::NumericVector& d_phi, Rcpp::NumericVector& mu_lambda, Rcpp::NumericVector& mu_phi, Rcpp::NumericVector& lambda_phi, Rcpp::NumericVector& bl_sum, Rcpp::NumericVector& tmp_j) 
    : Z(Z), mu_beta(mu_beta), dSigma(dSigma), mu_gamma(mu_gamma), Sigma_gamma(Sigma_gamma), dphi(dphi), muphi(muphi), mu_tau2(mu_tau2), mu_sigma(mu_sigma), mu_sigma2(mu_sigma2), a_lambda(a_lambda), b_lambda(b_lambda), c_lambda(c_lambda), d_phi(d_phi), mu_lambda(mu_lambda), mu_phi(mu_phi), lambda_phi(lambda_phi), bl_sum(bl_sum), tmp_j(tmp_j) {}
  ...
  void operator()(std::size_t k_0, std::size_t k_1) 
  {
    ...
    for (j = k_0; j < k_1; j++) 
    {
      ...
      a_lambda[j] = 0.5 * mu_beta2 * mu_tau2 * mu_sigma2;
      b_lambda[j] = 0.5 * muphi(j);
      c_lambda[j] = mu_j * muphi(j);
      
      double b_lambda_sq = std::sqrt(b_lambda[j]);
      // double lkf_p = std::log(1.0 - R::pnorm(0, mu_j, std::sqrt(2.0 * dphi(j)), true, false));
      // double lkf = 2.0 * log_kf(a_lambda[j], b_lambda_sq, c_lambda[j]);
      // double e1 = 2.0 * m_e1(a_lambda[j], b_lambda_sq, c_lambda[j], lkf);
      // double e2 = 2.0 * m_e2(a_lambda[j], b_lambda_sq, c_lambda[j], lkf);
      // double e3 = 2.0 * m_e3(a_lambda[j], b_lambda_sq, c_lambda[j], lkf);
      ...
      d_phi[j] = 0.5 + 0.5 * (e2 - 2.0 * e1 * mu_j + mu_j * mu_j + (zj.transpose() * Sigma_gamma * zj).value());
      
      mu_lambda[j+1] = e3;
      mu_phi[j] = 1.0 / d_phi[j];
      lambda_phi[j] = mu_phi[j] * e1;
      
      // tmp_j[j] = -lkf_p + lkf + 2.0 * a_lambda[j] * e3 + b_lambda[j] * e2 - c_lambda[j] * e1 - std::log(d_phi[j]);
      tmp_j[j] = lkf + 2.0 * a_lambda[j] * e3 + b_lambda[j] * e2 - c_lambda[j] * e1 - std::log(d_phi[j]);
...
std::tuple<Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd> ihs_pstep2 (const Eigen::MatrixXd& Z, const Eigen::VectorXd& mu_beta, const Eigen::VectorXd& dSigma, const Eigen::VectorXd& mu_gamma, const Eigen::MatrixXd& Sigma_gamma, const Eigen::VectorXd& dphi, const Eigen::VectorXd& muphi, const double& mu_tau2, const double& mu_sigma, const double& mu_sigma2) 
{
  ...
  ihs_parallel_step2 Ihs_parallel_step(Z, mu_beta, dSigma, mu_gamma, Sigma_gamma, dphi, muphi, mu_tau2, mu_sigma, mu_sigma2, a_lambda, b_lambda, c_lambda, d_phi, mu_lambda, mu_phi, lambda_phi, bl_sum, tmp_j);
  RcppParallel::parallelFor (0, p-1, Ihs_parallel_step);
...
struct ihs_pr_parallel_step0 : public RcppParallel::Worker {
  // function that return the updated inverse matrix after updating m columns
  // Q, R starting QR decomposition
  // k number of trials
  // X data matrix
  ...
  ihs_pr_parallel_step0 (const Eigen::MatrixXd& Z, const Eigen::VectorXd& mu_beta, const Eigen::VectorXd& dSigma, const Eigen::VectorXd& mu_gamma, const Eigen::MatrixXd& Sigma_gamma, const Eigen::VectorXd& dphi, const Eigen::VectorXd& muphi, const double& mu_tau2, Rcpp::NumericVector& a_lambda, Rcpp::NumericVector& b_lambda, Rcpp::NumericVector& c_lambda, Rcpp::NumericVector& d_phi, Rcpp::NumericVector& mu_lambda, Rcpp::NumericVector& mu_phi, Rcpp::NumericVector& lambda_phi, Rcpp::NumericVector& bl_sum, Rcpp::NumericVector& tmp_j) 
    : Z(Z), mu_beta(mu_beta), dSigma(dSigma), mu_gamma(mu_gamma), Sigma_gamma(Sigma_gamma), dphi(dphi), muphi(muphi), mu_tau2(mu_tau2), a_lambda(a_lambda), b_lambda(b_lambda), c_lambda(c_lambda), d_phi(d_phi), mu_lambda(mu_lambda), mu_phi(mu_phi), lambda_phi(lambda_phi), bl_sum(bl_sum), tmp_j(tmp_j) {}
...
std::tuple<Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd> ihs_pstep_pr2 (const Eigen::MatrixXd& Z, const Eigen::VectorXd& mu_beta, const Eigen::VectorXd& dSigma, const Eigen::VectorXd& mu_gamma, const Eigen::MatrixXd& Sigma_gamma, const Eigen::VectorXd& dphi, const Eigen::VectorXd& muphi, const double& mu_tau2) 
{
  ...
  ihs_pr_parallel_step0 Ihs_pr_parallel_step0(Z, mu_beta, dSigma, mu_gamma, Sigma_gamma, dphi, muphi, mu_tau2, a_lambda, b_lambda, c_lambda, d_phi, mu_lambda, mu_phi, lambda_phi, bl_sum, tmp_j);
  RcppParallel::parallelFor (0, p-1, Ihs_pr_parallel_step0);
