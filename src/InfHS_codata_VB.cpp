...
Rcpp::List infHS_VB (const Eigen::VectorXd& y, const Eigen::MatrixXd& X, const Rcpp::List Z_list, const int& M, const Eigen::VectorXd& hyp_sigma, const Eigen::VectorXd& a_k, const Eigen::VectorXd& b_k, const double& s0 = 1, const int& bmax = 1000, const double& eps = 1e-03, const int& ping = 100)
{
  ...
  // s0: optional prior scale parameter for local shrinkage parameters lambda, default equal to 1 (Horseshoe in Carvalho et al. (2010))
  ...
  const double s02inv = 1.0 / (2.0 * s0), p12 = 0.5 * ((double)p + 1.0), w_sigma2 = hyp_sigma(0) + 0.5 * double(n + p);
  ...
  double lkf_p, lkf, e1, e2, e3, tmp_j0;
  ...
  std::tie(a_lambda, b_lambda, c_lambda, d_phi, mu_lambda, mu_phi, lambda_phi, bl_sum, tmp_j) = ihs_pstep0(mu_beta, dSigma, s0, s02inv);
  ...
  for (d = 0; d < D; d++)
    mu_kappa.segment(md_index(d), md(d)) = repelem(s0 * a_k(d) / b_k(d), md(d));
  ...
  for (d = 0; d < D; d++)
  {
    ...
    for (m = md_index(d); m < md_index(d) + md(d); m++)
      f_kappa(d) += 0.5 * (std::pow(mu_gamma(m), 2.0) + s0 * Sigma_gamma(m, m));
  ...
  double l_sigma2 = hyp_sigma(1) + 0.5 * res.squaredNorm() + 0.5 * trace_xSx + 0.5 * bl_sum0 * mu_tau2;
  double mu_sigma2 = w_sigma2 / l_sigma2;
  
  // ::::::::::::::::::::::::::::::::::::::::::::::::
  // initialize ELBO 
  
  lb2 = tmp_j0 + tmp_d + 0.5 * (ldet_beta + ldet_gamma) - p12 * std::log(g_tau2) - std::log(h_zeta) - w_sigma2 * std::log(l_sigma2) + 0.5 * p * mean_ig_logx(w_sigma2, l_sigma2) - std::log(a0) - std::log(k0);
  ...
  while ((std::abs(lb2 - lb1) > eps) && (b < bmax))
  {
    ...
    a0 = 1.0 / k0 + 0.5 * mu_beta2 * mu_tau2 * mu_sigma2;
    ...
    std::tie(a_lambda, b_lambda, c_lambda, d_phi, mu_lambda, mu_phi, lambda_phi, bl_sum, tmp_j) = ihs_pstep(Z, mu_beta, dSigma, mu_gamma, Sigma_gamma, d_phi, mu_phi, mu_tau2, l_sigma2 / (w_sigma2 - 1.0), mu_sigma2, s0, s02inv);
    mu_lambda(0) = mu_lambda0;
    ...
    for (d = 0; d < D; d++)
      mu_kappa.segment(md_index(d), md(d)) = repelem(s0 * e_kappa(d) / f_kappa(d), md(d));
    ...
    g_tau2 = 0.5 * bl_sum0 * mu_sigma2 + 1.0 / h_zeta;
    mu_tau2 = p12 / g_tau2;
    ...
    l_sigma2 = hyp_sigma(1) + 0.5 * (res.squaredNorm() + trace_xSx + bl_sum0 * mu_tau2);
...
Rcpp::List infHS_VB_probit (const Eigen::VectorXd& y, const Eigen::MatrixXd& X, const Rcpp::List Z_list, const int& M, const Eigen::VectorXd& a_k, const Eigen::VectorXd& b_k, const double& s0 = 1, const int& bmax = 1000, const double& eps = 1e-03, const int& ping = 100)
{
  // y: numeric response variable, n-dim vector
  // X: design matrix, n x (p+1) matrix
  // Z_list: list of p x m_d co-data matrices, d = 1, ..., D, sum_d m_d = M
  ...
  const double s02inv = 1.0 / (2.0 * s0), s02inv_sqrt = std::sqrt(s02inv), p12 = 0.5 * ((double)p + 1.0);
  double lb1 = 1.0, lb2 = 0.0, tmp_d;
  ...
  while ((std::abs(lb2 - lb1) > eps) && (b < bmax))
  {
    ...
    std::tie(a_lambda, b_lambda, c_lambda, d_phi, mu_lambda, mu_phi, lambda_phi, bl_sum, tmp_j) = ihs_pstep_pr(Z, mu_beta, dSigma, mu_gamma, Sigma_gamma, d_phi, mu_phi, mu_tau2, s0, s02inv);
    mu_lambda(0) = mu_lambda0;
